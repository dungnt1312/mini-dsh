...
- **Phase 3 (done)** — tool pipeline: `ToolsService` with the guarded `tools/pre-execute` → execute → `tools/post-execute` path, approval policy (allow/ask/deny riding pre-execute), and the canonical built-in tools `Read`/`Write`/`Edit`/`Glob`/`Grep`/`Bash` (legacy lowercase names normalize at the boundary). The loop spends another step while tools owe the model their results, inside a configured step budget — the model decides when the turn ends. Verified end-to-end against the real DeepSeek API.
